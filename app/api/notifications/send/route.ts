...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, message, image, link, targetProvince, attachments } = body;
    ...
    const insertResult = await execute(
      `INSERT INTO notifications (title, message, image_url, attachments, link, sent_by_admin, target_province, status, sent_count, delivered_count, opened_count, sent_at)
